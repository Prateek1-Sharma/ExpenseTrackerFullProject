...
let token =localStorage.getItem("token");
  try{
let res=await axios.post("http://localhost:4000/expense/add/expenses",expenses,{headers:{"Authorization":token}})
...
async function getExpenses(page2){
  ...
  ul.innerHTML='';
  let token =localStorage.getItem("token");
 let result= await axios.get(`http://localhost:4000/expense/get/allExpenses?page=${page}`,{headers:{"Authorization":token}});
...
async function delExpense(id){
  ...
  const ul=document.getElementById("ul-list");
 try{
 let response=await axios.delete(`http://localhost:4000/expense/${id}/deleteExpenses`);
...
async function buyPremium(e){
  ...
  const token=localStorage.getItem("token");
  console.log(token);
  const res=await axios.get("http://localhost:4000/purchase/buyPremium",{headers:{"Authorization":token}});
  console.log(res);
...
inputElement.onclick=async()=>{
  const token=localStorage.getItem("token");
  const res=await axios.get(`http://localhost:4000/premium/showLeaderBoard`,{headers:{"Authorization":token}})
  console.log(res);
